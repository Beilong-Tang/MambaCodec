...
def main(rank, world_size, args):
    ...
    tr_data = DataLoader(
        ...
        shuffle=False,
        sampler=DistributedSampler(dataset=tr_dataset),
    )
    cv_data = DataLoader(
        cv_dataset,
        batch_size=config["batch_size"],
        shuffle=False,
        sampler=DistributedSampler(dataset=cv_dataset),
    )
    ### prepare model
    model_class = get_class("models", config["model"]["type"])
    model = model_class(
        **{**config["codec"], **config["model"], **{"device": device}}
    ).to(device)
    ...
    trainer = trainer_class(
        ...
        args,
        rank,
        get_attr(loss, config["loss"], "loss_fn"),
        logger,
    )
    trainer.train()
