...
class MseTrainer(AbsTrainer):
    ...
    def _train(self, loss_fn, optim, tr_data, epoch):
        ...
        for batch, (mix_audio, clean_audio) in enumerate(tr_data):
            ...
            if batch % self.log_interval == 0:
                ...
                self._log(f"epoch {epoch}, tr loss: {loss:>.7f}, pesq: {pesq:>.7f} , stoi: {stoi:>.7f}  [{current:>5d}/{(len(tr_data)*len(mix_audio)):>5d}], time: {(time.time() - start_time)*1000 :.2f}ms")
                start_time = time.time()
    ...
    def _eval(self, loss_fn, cv_data, epoch):
        ...
        self._log(f"epoch {epoch}, cv mse loss: {(mse_loss_avg) :>.7f}, pesq: {pesq_avg:>.7f}, stoi: {stoi_avg:>.7f}")
