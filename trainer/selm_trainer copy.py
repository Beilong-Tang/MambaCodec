...
class SelmTrainer(AbsTrainer):
    ...
    def _train(self, loss_fn, optim, tr_data, epoch):
        ...
        for batch, (mix_audio, clean_audio) in enumerate(tr_data):
            ...
            loss = self.kl_div_loss_fn(input_prob, true_token)
            loss.backward()
            optim.step()
            optim.zero_grad()
            ...
            if batch % self.log_interval == 0:
                ...
                self._log(
                    f"epoch {epoch}, tr kl loss: {kl_div_loss.item():>.7f}, time: {(time.time() - start_time)*1000 :.2f}ms"
                )
                start_time = time.time()
    ...
    def _eval(self, loss_fn, cv_data, epoch):
        ...
        with torch.no_grad():
            for batch, (mix_audio, clean_audio) in enumerate(cv_data):
                ...
                input_prob_max = torch.argmax(input_prob, dim=-1)  # [B,T]
                input_detokenize = self.model.detokenize(input_prob_max)  # [B, T, E]
                output_audio = self.model.decode_true(input_detokenize).squeeze(
                    1
                )  # output audio [B,T]
        ...
        self._log(
            f"epoch {epoch}, cv kl loss: {kl_div_loss_avg:>.7f}, mse loss {mse_loss_avg:>.7f}, mse de loss {mse_loss_de_avg :>.7f} si snr loss: {si_snr_loss_avg:>.7f}"
        )
